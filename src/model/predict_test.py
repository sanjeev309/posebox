...
if visualize_smoke_test:
    ...
    with open(config.OUTPUT + '/smoke_testing_results.csv') as csv_file:
        ...
        for row in csv_reader:
            ...
            cv2.putText(image, "1", (int(float(res[0])), int(float(res[1]))), cv2.FONT_HERSHEY_SIMPLEX, 1, [0, 0, 255],
                        1)
            cv2.putText(image, "2", (int(float(res[2])), int(float(res[3]))), cv2.FONT_HERSHEY_SIMPLEX, 1, [0, 0, 255],
                        1)
            cv2.putText(image, "3", (int(float(res[4])), int(float(res[5]))), cv2.FONT_HERSHEY_SIMPLEX, 1, [0, 0, 255],
                        1)
            cv2.putText(image, "4", (int(float(res[6])), int(float(res[7]))), cv2.FONT_HERSHEY_SIMPLEX, 1, [0, 0, 255],
                        1)
